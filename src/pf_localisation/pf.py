...
class PFLocaliser(PFLocaliserBase):
    ...
    def initialise_particle_cloud(self, initialpose):
        ...
        for i in range(numofParticles):
            ...
            pose.orientation = rotateQuaternion(initialpose.pose.pose.orientation, math.radians(
                np.random.uniform(0, 360)))  # might need to change to guassian noise
    ...
    def update_particle_cloud(self, scan):
        ...
        # Each particles Weight possibility
        for i in range(len(self.particlecloud.poses)):
            weights.append(self.sensor_model.get_weight(
        ...
        # set the new PoseArray to the self.particlecloud
        updatedPoseArray = PoseArray()

        # Resamping
        particlesWanted = numofParticles * 3
        cumSum = np.cumsum(normalisedWeights)
        uniformParticlesWanted = random.uniform(0, 1/particlesWanted)

        while j < particlesWanted:
            if (uniformParticlesWanted <= cumSum[i]):
                updatedPoseList.append(self.particlecloud.poses[i])
                j = j + 1
                uniformParticlesWanted = uniformParticlesWanted + 1 / particlesWanted
            else:
                i = i + 1
        ...
        for i in range(len(updatedPoseList)):
            updatedPose = Pose()
            # updatedPoseList[i].position.x # gauss(updatedPoseList[i].position.x, 0.1)
            updatedPose.position.x = random.gauss(
                updatedPoseList[i].position.x, 0.2)
            # updatedPoseList[i].position.y # gauss(updatedPoseList[i].position.y, 0.1)
            updatedPose.position.y = random.gauss(
                updatedPoseList[i].position.y, 0.2)
            # updatedPoseList[i].orientation #updatedPoseList[i].orientation # rotateQuaternion(updatedPoseList[i].orientation, math.radians(gauss(math.degrees(getHeading(updatedPoseList[i].orientation)),0.05)))
            updatedPose.orientation = rotateQuaternion(updatedPoseList[i].orientation, math.radians(
                random.gauss(getHeading(updatedPoseList[i].orientation), 5)))
            updatedPoseArray.poses.append(updatedPose)

        print("The sum weights: ", sum(weights)/numofParticles)
        ...
        # Tested 1 to 10, 10 is the best
        if (scatterIndex % 10 == 0):

            # 5% scatter particles
            for i in range(int(len(updatedPoseArray.poses) * scatterConstant)):
                midx = self.estimatedpose.pose.pose.position.x
                midy = self.estimatedpose.pose.pose.position.y
                randX = random.gauss(midx, 10)
                randY = random.gauss(midy, 10)
                ...
                for i in range(3):
                    ...
                    q_orig = [0, 0, 0, 1]
                    q_orig_msg = Quaternion(
                        q_orig[0], q_orig[1], q_orig[2], q_orig[3])
                    scatterPose.orientation = rotateQuaternion(
                        q_orig_msg, math.radians(np.random.uniform(0, 360)))

                    updatedPoseArray.poses.pop(
                        random.randrange(len(updatedPoseArray.poses)))
    ...
    def clustering(self):
        ...
        for i in range(0, self.M):
            ...
            d_yaw = (getHeading(
                self.particlecloud.poses[i].orientation) - s_yaw)
